...
def main():
    ...
    idx = ranking_df.groupby(['Essential_gene_set', 'Project', 'Subsampling'])['lib_dep_RT_score'].transform(max) == ranking_df['lib_dep_RT_score']
    ...
    for set_idx in range(0, len(ranking_df)):
        ...
        RT_ess.to_csv(os.path.join(options.output, essential_gene_set + '_' + project + '_' + subsampling + '_ess.tsv'), sep='\t', index=False)
        RT_noness.to_csv(os.path.join(options.output, essential_gene_set + '_' + project + '_' + subsampling + '_noness.tsv'), sep='\t', index=False)
